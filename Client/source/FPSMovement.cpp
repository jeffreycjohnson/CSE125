	...
	*/
	...
	if (oct != nullptr) {
		...
		if (moveHit.intersects && moveHit.hitTime >= 0) {
			std::cout << "intersect = " << moveHit.intersects << ", time = " << moveHit.hitTime << ", normal:" << moveHit.normal.x << ", " << moveHit.normal.z << std::endl;

			std::cout << "radius = " << playerRadiusTime << std::endl;
		...
		if (moveHit.intersects && moveHit.hitTime <= playerRadiusTime && moveHit.hitTime >= 0) {
			...
			if (moveHit.normal.z != 0)
				newMoveVec.z = 0;
			//glm::vec3 newMoveVec = glm::normalize(moveDir)*moveHit.hitTime
